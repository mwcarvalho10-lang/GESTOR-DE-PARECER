...
export function PinModal({ isOpen, targetGrade, targetLetter, currentPin, onPinChange, onCancel }: PinModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[10000]">
      <div className="bg-white rounded-3xl p-8 shadow-2xl w-full max-w-[340px] border-t-8 border-escola-azul">
        ...
        <div className="flex justify-center gap-4 mb-8">
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={i} className={`w-3 h-3 rounded-full border-2 ${i < currentPin.length ? 'bg-escola-azul border-escola-azul scale-125' : 'border-slate-300'} transition-all`} />
          ))}
        </div>
